...
임의의 4차원 벡터 $$ a $$는 아인슈타인 표기로는 $$ \newcommand{\a}{A^{\mu}e*{\mu}} \a $$라고 표기하며, 여기서 $$ \mu $$는 더미 인덱스<sub>dummy index</sub>로, 위 첨자와 아래 첨자에 동시에 등장한다면 반복하며 곱해서 더하라는 뜻을 가집니다. 다시 말해 4차원 벡터일 경우 $$ \a = \sum*{i}A^ie_i = A^0e_0 + A^1e_1 + A^2e_2 + A^3e_3 $$입니다.
